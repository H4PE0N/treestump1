...
bool move_pattern_valid(Move move, Piece piece)
{
	...
	{
		return pawn_pattern_handler(move, pieceTeam);
	}
	else if(pieceType == PIECE_TYPE_KNIGHT)
	{
		return knight_pattern_handler(move, pieceTeam);
	}
	else if(pieceType == PIECE_TYPE_BISHOP)
	{
		return bishop_pattern_handler(move, pieceTeam);
	}
	else if(pieceType == PIECE_TYPE_ROOK)
	{
		return rook_pattern_handler(move, pieceTeam);
	}
	else if(pieceType == PIECE_TYPE_QUEEN)
	{
		return queen_pattern_handler(move, pieceTeam);
	}
	else if(pieceType == PIECE_TYPE_KING)
	{
		return king_pattern_handler(move, pieceTeam);
	}
	else return false;
}
...
bool knight_pattern_handler(Move move, Piece pieceTeam)
{
	Move moveFlag = (move & MOVE_FLAG_MASK);
...
bool bishop_pattern_handler(Move move, Piece pieceTeam)
{
	Move moveFlag = (move & MOVE_FLAG_MASK);
...
bool rook_pattern_handler(Move move, Piece pieceTeam)
{
	Move moveFlag = (move & MOVE_FLAG_MASK);
...
bool queen_pattern_handler(Move move, Piece pieceTeam)
{
	Move moveFlag = (move & MOVE_FLAG_MASK);
...
bool king_pattern_handler(Move move, Piece pieceTeam)
{
	Move moveFlag = (move & MOVE_FLAG_MASK);

	if(moveFlag == MOVE_FLAG_CASTLE)
	{
		return castle_pattern_valid(move, pieceTeam);
	}
	else if(moveFlag == MOVE_FLAG_NONE)
	{
...
bool castle_pattern_valid(Move move, Piece pieceTeam)
{
...
bool double_pattern_valid(Move move, Piece pieceTeam)
{
	unsigned short fileOffset = ABS_SHORT_NUMBER(move_file_offset(move, pieceTeam));
	signed short rankOffset = move_rank_offset(move, pieceTeam);
...
bool passant_pattern_valid(Move move, Piece pieceTeam)
{
	unsigned short fileOffset = ABS_SHORT_NUMBER(move_file_offset(move, pieceTeam));
	signed short rankOffset = move_rank_offset(move, pieceTeam);
...
bool promote_pattern_valid(Move move, Piece pieceTeam)
{
