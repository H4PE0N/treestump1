...
bool render_chess_board(Screen, const Piece[], State, const Move[]);

bool render_mark_board(Screen, const Piece[], State, const Move[], const Point[]);

bool render_move_board(Screen, const Piece[], State, const Move[], Point);

bool render_promote_board(Screen, uint8_t);

bool render_result_board(Screen, const Piece[], State);

// ####################################################################################

bool render_move_squares(Screen, const Piece[], State, Point);

bool render_latest_move(Screen, const Move[]);

bool render_engine_moves(Screen, const Move[]);

bool render_check_squares(Screen, const Piece[], State);

bool render_board_pieces(Screen, const Piece[]);

bool render_board_piece(Screen, Piece, Point);

bool render_check_square(Screen, const Piece[], State, Point);
